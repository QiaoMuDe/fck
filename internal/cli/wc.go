...
func init() {
	...
	cmdOpts := &qflag.CmdOpts{
		...
		Examples: map[string]string{
			"默认统计（行数、单词、字节）": "fck wc file.txt",
			"仅统计行数":           "fck wc -l file.txt",
			"统计行数和单词数":        "fck wc -lw file.txt",
			"统计字符数 (Unicode)": "fck wc -m file.txt",
			"统计最大行长度":         "fck wc -L file.txt",
			"多文件统计":           "fck wc file1.txt file2.txt",
			"通配符支持":           "fck wc *.txt",
			"管道输入":            "cat file.txt | fck wc",
			"统计代码行数":          "fck wc -l *.go",
		},
		Notes: []string{
			"无标志时默认输出：行数、单词数、字节数",
