...
func init() {
	...
	cmdOpts := &qflag.CmdOpts{
		...
		Examples: map[string]string{
			"打印第1列":      fmt.Sprintf("%s awk -f 1 file.txt", qflag.Root.Name()),
			"打印第1和第3列":   fmt.Sprintf("%s awk -f 1,3 file.txt", qflag.Root.Name()),
			"指定分隔符":      fmt.Sprintf("%s awk -F: -f 1,3 /etc/passwd", qflag.Root.Name()),
			"打印最后一列":     fmt.Sprintf("%s awk -f -1 file.txt", qflag.Root.Name()),
			"正则匹配并打印":    fmt.Sprintf("%s awk -p \"error\" -f 0 log.txt", qflag.Root.Name()),
			"显示行号":       fmt.Sprintf("%s awk -n -f 1 file.txt", qflag.Root.Name()),
			"CSV处理":      fmt.Sprintf("%s awk -F, -f 1,3 data.csv", qflag.Root.Name()),
			"制表符分隔":      fmt.Sprintf("%s awk -F\"\\t\" -f 2 file.tsv", qflag.Root.Name()),
			"自定义输出格式":    fmt.Sprintf("%s awk -F: -O\" -> \" -f 1,6 /etc/passwd", qflag.Root.Name()),
			"管道输入":       fmt.Sprintf("%s awk -f 1 cat file.txt", qflag.Root.Name()),
			"提取前10个字符":   fmt.Sprintf("%s awk -c 1-10 file.txt", qflag.Root.Name()),
			"提取第5个字符到行尾": fmt.Sprintf("%s awk -c 5- file.txt", qflag.Root.Name()),
			"提取多个字符范围":   fmt.Sprintf("%s awk -c 1,5-10,15 file.txt", qflag.Root.Name()),
		},
		Notes: []string{
