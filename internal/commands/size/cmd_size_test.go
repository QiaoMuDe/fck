...
func TestGetPathSize(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			size, err := getPathSize(tt.path, config)
...
func TestGetPathSizeWithHiddenFiles(t *testing.T) {
	...
	t.Run("不包含隐藏文件", func(t *testing.T) {
		...
		size, err := getPathSize(tempDir, config)
		if err != nil {
		...
		if size != expectedSize {
			t.Errorf("getPathSize 返回大小 %d，期望 %d", size, expectedSize)
		}
	})

	// 测试包含隐藏文件的情况
	t.Run("包含隐藏文件", func(t *testing.T) {
		config := SizeConfig{
			Hidden: true,
		}
		size, err := getPathSize(tempDir, config)
		...
		// 应该包含普通文件和隐藏文件的大小
		expectedSize := int64(12) // "normal" + "hidden" = 6 + 6 = 12 bytes
		if size != expectedSize {
			t.Errorf("getPathSize 返回大小 %d，期望 %d", size, expectedSize)
		}
	})
...
func BenchmarkGetPathSize(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := getPathSize(testFile, config)
		if err != nil {
