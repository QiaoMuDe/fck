...
func (s *FileScanner) scanDirectoryWithOriginal(dirPath, rootDir string, originalPath string, opts ScanOptions) (FileInfoList, error) {
	...
	if opts.DirItself {
		...
		fileInfo := s.buildFileInfoWithOriginal(pathInfo, dirPath, originalPath, opts.Recursive)
		return FileInfoList{fileInfo}, nil
	}
	...
	for _, entry := range entries {
		...
		info := s.buildFileInfoWithOriginal(fileInfo, absEntryPath, originalPath, opts.Recursive)
...
// 返回:
//   - FileInfo: 文件信息
func (s *FileScanner) buildFileInfoWithOriginal(fileInfo os.FileInfo, absPath string, originalPath string, recursive bool) FileInfo {
	var baseName string
	if recursive {
		baseName = absPath
	} else {
