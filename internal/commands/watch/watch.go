...
func (c *WatchConfig) Validate() error {
	...
	// 累积模式自动启用差异高亮
	if c.Cumulative && c.Diff == DiffModeNone {
		c.Diff = DiffModeCumulative
	}
	return nil
}
...
func (r *Runner) Run(ctx context.Context) error {
	...
	for {
		...
		if err != nil {
			...
			// 检查变化并响铃
			if r.config.BeepOnChange && lastOutput != result.Stdout {
				r.output.Beep()
			}
