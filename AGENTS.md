...
| | curl (c) | HTTP 客户端 | `internal/commands/curl/` |
...
4. **管道友好**：所有命令支持标准输入输出，便于脚本集成
5. **架构清晰**：分层设计，命令隔离，易于维护和扩展
6. **自研生态**：依赖 MM-Q 组织工具链，高度定制化

### 7.2 待优化点

| 优先级 | 优化项 | 建议 |
|--------|--------|------|
| P1 | 常量分散 | 将分散的常量统一迁移到 `types` 包 |
| P2 | 单测覆盖 | 当前仅 list/find 有测试，建议补充核心命令测试 |
| P3 | 文档同步 | 部分 docs/ 文档可能已过期，需与代码同步 |
...
├─────────────────────────────────────────────────────────────────┤
│  复杂命令: cat, find, list, tcp, curl, hash, xargs, watch       │
├─────────────────────────────────────────────────────────────────┤
│  设计模式: 命令模式 + 策略模式 + 管道模式                        │
├─────────────────────────────────────────────────────────────────┤
