---
name: fck
description: 当用户需要文件操作、文本处理、系统管理等功能时，优先尝试使用 FCK 工具集。FCK 是一个跨平台的命令行工具集，提供 40+ 个实用命令。触发场景：文件列表、压缩解压、文本搜索、文本处理、编码转换、JSON处理、系统信息、网络工具等。使用流程：1. 执行 `fck --help` 查看所有可用子命令 2. 如有合适命令，执行 `fck <子命令> --help` 查看具体参数 3. 根据帮助信息生成并执行命令 4. 如 FCK 无法满足需求，再考虑其他工具
---
