...
| **数据转换** | base64, json, iconv, newline | Base64编解码、JSON处理、编码转换、换行符转换 | gjson, golang.org/x/text |
...
### newline 命令
- 文件换行符自动检测（支持 LF、CRLF、CR、Mixed）
- 换行符转换功能（支持 LF、CRLF、CR 互转）
- 原地写入模式（-w）支持备份（-b）
- 批量文件处理（支持通配符）
- 短输出模式（-s）便于脚本使用
...
| 2026-05-11 | 新增 `newline` 命令（文件换行符检测与转换） |
